#!/usr/bin/env python3
"""
Test data preparation script for creating manufacturer-specific class splits.
...
Output structure:
    data/processed/test/fork_a/
    data/processed/test/fork_b/
    data/processed/test/knife_a/
    data/processed/test/knife_b/
    data/processed/test/spoon_a/
    data/processed/test/spoon_b/
...
def setup_output_dirs() -> None:
    ...
    for class_name in CLASSES:
        for suffix in ["a", "b"]:
            target_dir = TEST_DIR / f"{class_name}_{suffix}"
...
def process_test_data(num_images: int) -> Dict[str, int]:
    """
    Process raw data into manufacturer-specific test splits.
    ...
    for i, manufacturer in enumerate(MANUFACTURERS):
        ...
        for class_name in CLASSES:
            ...
            logger.info(
                f"Copied {count} images for testing: {source_dir} → {target_dir}"
            )

    return stats


def verify_test_split(stats: Dict[str, int]) -> None:
    """
    Verify that test folders contain the expected images.
    ...
    for class_name in CLASSES:
        for suffix in ["a", "b"]:
            test_dir = TEST_DIR / f"{class_name}_{suffix}"
            actual_count = len(list(get_image_files(test_dir)))
...
def main():
    """Main function to prepare manufacturer-specific test split."""
    parser = argparse.ArgumentParser(
        description="Prepare test split with manufacturer-specific classes"
    )
    parser.add_argument(
        "--num-images",
        type=int,
    ...
    try:
        ...
        # Process test data
        stats = process_test_data(args.num_images)

        # Verify the split
        verify_test_split(stats)
        ...
        logger.error(f"Error during test data preparation: {str(e)}")
        raise
